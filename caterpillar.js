var caterpillar_colors = "ffb5a7-fcd5ce-f8edeb-f9dcc4-fec89a".split("-").map(a=>"#"+a)
        ...
        if(this.dead==false){
            ...
            push() //重新設定圓點位置
                ...
                beginShape()
                    ...
                    ellipse(this.p.x/2+165, this.p.y/2+70, 64, 64)
                    ellipse(this.p.x/2+120, this.p.y/2+90, 66, 66)
                    ellipse(this.p.x/2+70, this.p.y/2+70, 88, 88)
                    ellipse(this.p.x/2+20, this.p.y/2+30, 100, 100)
                    ellipse(this.p.x/2+10, this.p.y/2-15, 150, 150)
        
                endShape();
                //+++++++++++++++++++++++++
                if(this.mode=="big"){
                    fill("black")
                    ellipse(this.p.x/2+10+map(mouseX,0,width,-10,10),this.p.y/2-15+map(mouseY,0,this.p.y,-10,10),30) //眼睛跟著滑鼠位置做移動
                    ellipse(this.p.x/2+30+map(mouseX,0,width,-10,10),this.p.y/2-15+map(mouseY,0,this.p.y,-10,10),30) //眼睛跟著滑鼠位置做移動
                }else{
                    fill("#fae0e4")
            ...
            push()
                translate(this.p.x,this.p.y) //把原點(0,0)座標移動到物件中心位置
                fill("#ffd6ff")  //填滿this.color其中一個顏色
                //ellipse(0,0,this.r)
                strokeWeight(4)  //線條粗細
                noFill()  //不填滿
                fill("#FFFFFF")
